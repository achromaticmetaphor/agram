...
int agram_wl_upgrade(const char * const fnold, const char * const fnnew, char const version)
{
  ...
  int const error = !wl_loaded || duplicate(fnold, fnnew, 's', wl.strbase, wl.strbase_len) || duplicate(fnold, fnnew, 'c', wl.charsbase, wl.charsbase_len) || duplicate(fnold, fnnew, 'n', wl.countsbase, wl.countsbase_len) || upgrade_lcs(&wl, fnnew) || write_base(&wl, fnnew);
  if (wl_loaded)
    unload_wl(&wl);
  return error;
}
